/*
 * Qmi8658.h - IMU library for Arduino - Version 1.0
 *
...
/* Status Registers */
#define QMI8658_STATUSINT   0x2D  // Sensor Data Available and Lock Register Address
#define QMI8658_STATUS0     0x2E  // Output Data Status Register Address
#define QMI8658_STATUS1     0x2F  // Miscellaneous Status. Register Address
...
typedef enum {
    acc_odr_8000,      // Accelerometer ODR set to 7520 Hz. (6DOF only)
    acc_odr_4000,      // Accelerometer ODR set to 3760 Hz. (6DOF only)
    acc_odr_2000,      // Accelerometer ODR set to 1880 Hz. (6DOF only)
    acc_odr_1000,      // Accelerometer ODR set to 1000/940 Hz.
    acc_odr_500,       // Accelerometer ODR set to 500/470 Hz.
    acc_odr_250,       // Accelerometer ODR set to 250/235 Hz.
    acc_odr_125,       // Accelerometer ODR set to 125/117.5 Hz.
    acc_odr_62_5,      // Accelerometer ODR set to 62.5/58.75 Hz.
    acc_odr_31_25,     // Accelerometer ODR set to 31.25/29.375 Hz.
    acc_odr_128 = 12,  // Accelerometer ODR set to 128 Hz. (Accel only)
    acc_odr_21,        // Accelerometer ODR set to 21 Hz. (Accel only)
    acc_odr_11,        // Accelerometer ODR set to 11 Hz. (Accel only)
    acc_odr_3,         // Accelerometer ODR set to 3 Hz. (Accel only)
} acc_odr_t;

/* Enum representing the output data rate (ODR) settings for the gyroscope */
typedef enum {
    gyro_odr_7520,     // Gyroscope ODR set to 7520 Hz.
    gyro_odr_3760,     // Gyroscope ODR set to 3760 Hz.
    gyro_odr_1880,     // Gyroscope ODR set to 1880 Hz.
    gyro_odr_940,      // Gyroscope ODR set to 940 Hz.
    gyro_odr_470,      // Gyroscope ODR set to 470 Hz.
    gyro_odr_235,      // Gyroscope ODR set to 235 Hz.
    gyro_odr_117,      // Gyroscope ODR set to 117.5 Hz.
    gyro_odr_58,       // Gyroscope ODR set to 58.75 Hz.
    gyro_odr_29,       // Gyroscope ODR set to 29.375Hz.
} gyro_odr_t;
...
/* Struct representing the data read from Qmi8658 */
typedef struct {
    acc_axes_t  acc_xyz;       // Accelerometer data in three axes (x, y, z).
    gyro_axes_t gyro_xyz;      // Gyroscope data in three axes (x, y, z).
    float temperature;         // Temperature reading from Qmi8658.
} qmi_data_t;

/* Enum representing the mode of operation for Qmi8658 */
typedef enum {
...
/* Struct representing the configuration settings for Qmi8658 */
typedef struct {
    qmi8658_mode_t qmi8658_mode;    // Mode of operation for Qmi8658.
...
/* Enum representing the result of an operation with Qmi8658 */
typedef enum {
    qmi8658_result_open_success,   // Operation to open communication with Qmi8658 was successful.
    qmi8658_result_open_error,     // Error occurred while trying to open communication with Qmi8658.
    qmi8658_result_close_success,  // Operation to close communication with Qmi8658 was successful.
    qmi8658_result_close_error,    // Error occurred while trying to close communication with Qmi8658.
} qmi8658_result_t;
...
{
public:
    uint8_t deviceID;                                         // Device ID of the Qmi8658.
    uint8_t deviceRevisionID;                                 // Revision ID of the Qmi8658.
    
private:
    uint8_t deviceAdress;                                     // Device address of the Qmi8658.
    uint16_t deviceFrequency;                                 // Frequency of the Qmi8658.
    acc_axes_t  acc_offset_xyz;                               // Accelerometer offset in three axes (x, y, z).
    gyro_axes_t gyro_offset_xyz;                              // Gyroscope offset in three axes (x, y, z).
    
public:
    Qmi8658(uint8_t deviceAdress, uint32_t deviceFrequency);  // Constructor for Qmi8658 class.
    qmi8658_result_t open(qmi8658_cfg_t* qmi8658_cfg);        // Open communication with the Qmi8658 and configures it.
    void raw_read(qmi_data_t* data);                          // Read raw data from the Qmi8658.
    void acc_offset(void);                                    // Set Accelerometer offset
    void gyro_offset(void);                                   // Set Gyroscope offset
    void read(qmi_data_t* data);                              // Read data from the Qmi8658.
    qmi8658_result_t close(void);                             // Close communication with the Qmi8658.
    const char* resultToString(qmi8658_result_t result);      // Convert a qmi8658_result_t enum value into a corresponding string representation.
    uint8_t qmi_status0(void);
    uint8_t qmi_status1(void);

private:
    void qmi8658_write(uint8_t reg,uint8_t value);            // Write a value to a register of the Qmi8658.    
    uint8_t qmi8658_read(uint8_t reg);                        // Read a value from a register of the Qmi8658.   
    void qmi_reset(void);                                     // Reset the Qmi8658.
    void select_mode(qmi8658_mode_t qmi8658_mode);            // Select the mode of the Qmi8658.
